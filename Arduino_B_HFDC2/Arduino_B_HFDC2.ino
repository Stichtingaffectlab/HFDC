...
float p2s1; //Person 1, sensor 1 // change accordingly
float trigger;
float prevTemp = 0;
...
void loop() {
  // Read Temperature
  p2s1 = mcp9808.getTemperature();
  int scaledValLED = map(p2s1, 29, 35, 0, 255);  // scale to 0-255
...
if (Baseline0_2 == 0){
  ...
  if (p2s1 > highestTemp) {
    ...
    trigger = 1;
    OscWiFi.send(destIp.c_str(), destPort, "/trigger/wearableB", trigger); //triggers based on data
  } else if (p2s1 < lowestTemp) {
  ...
  if (p2s1 != prevTemp && p2s1 != 0) { //if p1 is not equal to prevtemp and not equal to 0, we sent the data and reset prevtemp values
    OscWiFi.send(destIp.c_str(), destPort, "/arduino2/sensordata", p2s1); //Wearable 1, Person 1, sensor 1 // change accordingly
    prevTemp = p2s1;
    count = 0;
    trigger = 0;
    OscWiFi.send(destIp.c_str(), destPort, "/trigger/wearableB", trigger); //triggers based on data
    Serial.println(p2s1);
  } else if (p2s1 == prevTemp || p2s1 == 0) { // if p1 is equal to prevtemp or 0, we count how long, and sent a trigger if change is not happening
    count++;
...
void calibration(){
  ...
  Baseline0_2 = total / numReadings;
   Threshold2 = Baseline0_2 + 3; // Dynamic threshold initialization
  Serial.print("Baseline: ");
  Serial.println(Baseline0_2);
  Serial.print("Initial Threshold: ");
  Serial.println(Threshold2);

  OscWiFi.send(destIp.c_str(), destPort, "/arduino2/Baseline0_2", Baseline0_2); //triggers based on data
}
