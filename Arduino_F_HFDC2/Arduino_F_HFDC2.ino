...
//
//const char* ssid     = "Effenaar-Artist";
//const char* password = "@rtistwifi";
//const char* destIp   = "10.99.20.85";
//const int destPort   = 8000;


// Wearable sensor data
float p6s1; //Person 1, sensor 1 // change accordingly
float trigger;
float prevTemp = 0;
...
void loop() {
  // Read Temperature
  p6s1 = mcp9808.getTemperature();
  int scaledValLED = map(p6s1, Baseline0_6, Threshold2, 0, 255);  // scale to 0-255
  ...
  if (p6s1 > highestTemp) {
    ...
    trigger = 1;
    OscWiFi.send(destIp.c_str(), destPort, "/trigger/wearableF", trigger); //triggers based on data
  } else if (p6s1 < lowestTemp) {
  ...
  if (p6s1 != prevTemp && p6s1 != 0) { //if p1 is not equal to prevtemp and not equal to 0, we sent the data and reset prevtemp values
    OscWiFi.send(destIp.c_str(), destPort, "/arduino6/sensordata", p6s1); //Wearable 1, Person 1, sensor 1 // change accordingly
    prevTemp = p6s1;
    count = 0;
    trigger = 0;
    OscWiFi.send(destIp.c_str(), destPort, "/trigger/wearableF", trigger); //triggers based on data
    Serial.println(p6s1);
  } else if (p6s1 == prevTemp || p6s1 == 0) { // if p1 is equal to prevtemp or 0, we count how long, and sent a trigger if change is not happening
    count++;
...
if (p6s1 <= (Baseline0_6 - 6)){
  Serial.println("Below, 26 New calibration needed");
  delay(3000);
  calibration();
}
  ...
  //     OscWiFi.send(destIp, destPort, "/trigger/wearableF", trigger); //triggers based on data
...
void calibration(){
  ...
  Baseline0_6 = total / numReadings;
   Threshold2 = Baseline0_6 + 3; // Dynamic threshold initialization
  Serial.print("Baseline: ");
  Serial.println(Baseline0_6);
  Serial.print("Initial Threshold: ");
  Serial.println(Threshold2);

   OscWiFi.send(destIp.c_str(), destPort, "/arduino6/Baseline0_6", Baseline0_6); //triggers based on data
