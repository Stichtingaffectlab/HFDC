...
float p5s1; //Person 1, sensor 1 // change accordingly
float trigger;
float prevTemp = 0;
...
void loop() {
  // Read Temperature
  p5s1 = mcp9808.getTemperature();
  int scaledValLED = map(p5s1, Baseline0_5, Threshold2, 0, 255);  // scale to 0-255
...
if (Baseline0_5 == 0){
  ...
  if (p5s1 > highestTemp) {
    ...
    trigger = 1;
    OscWiFi.send(destIp.c_str(), destPort, "/trigger/wearableE", trigger); //triggers based on data
  } else if (p5s1 < lowestTemp) {
  ...
  if (p5s1 != prevTemp && p5s1 != 0) { //if p1 is not equal to prevtemp and not equal to 0, we sent the data and reset prevtemp values
    OscWiFi.send(destIp.c_str(), destPort, "/arduino5/sensordata", p5s1); //Wearable 1, Person 1, sensor 1 // change accordingly
    prevTemp = p5s1;
    count = 0;
    trigger = 0;
    OscWiFi.send(destIp.c_str(), destPort, "/trigger/wearableE", trigger); //triggers based on data
    Serial.println(p5s1);
  } else if (p5s1 == prevTemp || p5s1 == 0) { // if p1 is equal to prevtemp or 0, we count how long, and sent a trigger if change is not happening
    count++;
...
if (p5s1 <= (Baseline0_5 - 6)){
  ...
  //     OscWiFi.send(destIp, destPort, "/trigger/wearableE", trigger); //triggers based on data
...
void calibration(){
  ...
  Baseline0_5 = total / numReadings;
   Threshold2 = Baseline0_5 + 3; // Dynamic threshold initialization
  Serial.print("Baseline: ");
  Serial.println(Baseline0_5);
  Serial.print("Initial Threshold: ");
  Serial.println(Threshold2);

   OscWiFi.send(destIp.c_str(), destPort, "/arduino5/Baseline0_5", Baseline0_5); //triggers based on data
}
