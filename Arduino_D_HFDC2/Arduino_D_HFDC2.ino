...
float p4s1; //Person 1, sensor 1 // change accordingly
float trigger;
float prevTemp = 0;
...
//  Serial.println(destIp);
//  
  p4s1 = mcp9808.getTemperature();
  int scaledValLED = map(p4s1, Baseline0_4, Threshold2, 0, 255);  // scale to 0-255
  ...
  if (p4s1 > highestTemp) {
    ...
    trigger = 1;
    OscWiFi.send(destIp, destPort, "/trigger/wearableD", trigger); //triggers based on data
  } else if (p4s1 < lowestTemp) {
  ...
  if (p4s1 != prevTemp && p4s1 != 0) { //if p1 is not equal to prevtemp and not equal to 0, we sent the data and reset prevtemp values
    OscWiFi.send(destIp, destPort, "/arduino4/sensordata", p4s1); //Wearable 1, Person 1, sensor 1 // change accordingly
    prevTemp = p4s1;
    count = 0;
    trigger = 0;
    OscWiFi.send(destIp, destPort, "/trigger/wearableD", trigger); //triggers based on data
    OscWiFi.send(destIp, destPort, "/arduino4/Baseline0_4", Baseline0_4); //triggers based on data
    Serial.println(p4s1);
  } else if (p4s1 == prevTemp || p4s1 == 0) { // if p1 is equal to prevtemp or 0, we count how long, and sent a trigger if change is not happening
    count++;
...
void calibration(){
  ...
  Baseline0_4 = total / numReadings;
   Threshold2 = Baseline0_4 + 3; // Dynamic threshold initialization
  Serial.print("Baseline: ");
  Serial.println(Baseline0_4);
